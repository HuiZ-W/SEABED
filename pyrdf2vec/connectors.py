...
class SPARQLConnector(Connector):
    ...
    async def afetch(self, queries: List[str]) -> List[List[Response]]:
        ...
        for i in tqdm(range(0, len(queries), batch_size)):
            ...
            responses.extend(await asyncio.gather(*(self._fetch(query) for query in batch_queries)))
        return responses
        #return await asyncio.gather(*(self._fetch_with_semaphore(query) for query in queries))
    ...
    async def _fetch(self, query) -> Response:
        ...
        """
        query_params = params
        query_params["sparql"] = query
        url = f"{self.endpoint}"
        try:
            async with self._asession.get(url, params=query_params) as res:
    ...
    def fetch(self, query: str) -> Response:
        ...
        query_params["sparql"] = query
        url = f"{self.endpoint}"
        with requests.get(url, params=query_params) as res:
            return res.json()
